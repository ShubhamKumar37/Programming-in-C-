...
namespace CustomInterface
{
    ...
    public class Hexagon : Shape, IDraw3D, IPointy
    {
        public Hexagon() { }
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Examle Property {IRegularlyPointy.ExampleProperty}");
            Shape[] myShapes = { new Hexagon(), new Circle(), new Triangle("Joe"), new ThreeDCircle() };
            for (int a = 0; a < myShapes.Length; a++)
            {
                // Can I draw you in 3D?
                if (myShapes[a] is IDraw3D s)
                {
                    Console.WriteLine(myShapes[a].ToString());
                    DrawIn3D(s);
                }
            }

            Console.WriteLine(FindFirstIPointyRef(myShapes));

            IPointy[] myPointyObjects = {new Hexagon(), new Knife(), new Triangle(), new Fork(), new PitchFork()};
            foreach (IPointy k in myPointyObjects)
            {
                Console.WriteLine("Object has {0} points.", k.Points);
            }
            Console.ReadLine();
        ...
        static void DrawIn3D(IDraw3D id3d)
        {
            Console.WriteLine("This is a drawing tool");
            id3d.Draw3D();
        }

        static IPointy FindFirstIPointyRef(Shape[] sp)
        {
            foreach (Shape sh in sp)
            {
                if (sh is IPointy i) return i;
            }
            return null;
        }
    }
